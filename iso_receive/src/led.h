...
#define TTL_PWM_LED_PERIODE 100000
#define TTL_PWM_LED_DUTY (TTL_PWM_LED_PERIODE / 2)
 ...
 */
static void inline ttl_led_connect(const struct pwm_dt_spec *pwm)
{
    nrfx_pwm_t *nrfx_pwm = (nrfx_pwm_t *)pwm->dev->config;
    nrfx_pwm->p_reg->PSEL.OUT[pwm->channel] &= ~(0x1 << PWM_PSEL_OUT_CONNECT_Pos);
}

static void inline ttl_led_disconnect(const struct pwm_dt_spec *pwm)
{
    nrfx_pwm_t *nrfx_pwm = (nrfx_pwm_t *)pwm->dev->config;
    nrfx_pwm->p_reg->PSEL.OUT[pwm->channel] |= (0x1 << PWM_PSEL_OUT_CONNECT_Pos);
}
